...
def sidebar_and_documentChooser():
    ...
    with tab_open_vectorstore:
        ...
        if LOCAL_VECTOR_STORE_DIR.exists():
            for item in sorted(LOCAL_VECTOR_STORE_DIR.iterdir()):
                if item.is_dir() and (item / "chroma.sqlite3").exists():
                    vectorstore_dirs.append(item)

        st.session_state.selected_vectorstore_name = ""

        if not vectorstore_dirs:
            st.info("No saved vectorstores found. Create one in the other tab first.")
        else:
            vectorstore_names = [d.name for d in vectorstore_dirs]
            selected_name = st.selectbox("Available vectorstores", vectorstore_names)
            ...
            if st.button("Load Vectorstore"):
                ...
                if len(error_messages) == 1:
                    ...
                    selected_vectorstore_path = LOCAL_VECTOR_STORE_DIR / selected_name
                    st.session_state.selected_vectorstore_name = selected_name
                    ...
                    with st.spinner("Loading vectorstore..."):
                        try:
                            ...
                            st.info(
                                f"**{selected_name}** is loaded successfully."
                            )
...
def select_embeddings_model():
    ...
    if st.session_state.LLM_provider == "Google":
        embeddings = GoogleGenerativeAIEmbeddings(
            model="models/gemini-embedding-001", google_api_key=st.session_state.google_api_key
        )
        try:
            embeddings.embed_query("validation test")
        except Exception as e:
            st.error(f"Google embedding model unavailable: {e}")
            st.stop()
...
def chain_RAG_blocks():
    ...
    with st.spinner("Creating vectorstore..."):
        ...
        if len(error_messages) == 1:
            ...
            st.session_state.error_message = ""
            try:
                # 1. Ensure temp directory exists and clean it
